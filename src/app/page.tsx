...
export default function Home() {
  ...
  const messages = [
    ...
    ...thormails.filter(message => !message.content || !(message.content === 'test')).map((mail, i) => ({
      id: i + 1,
      isCompose: false,
      title: `${mail.from === walletAddress ? 'Me' : mail.from.slice(-4)} → ${
        mail.recipient === THORMAIL_ADDRESS 
          ? 'All' 
          : mail.recipient === walletAddress ? 'Me' : mail.recipient.slice(-4)
      }`,
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      ...
      <CursorTrail />
      <div className="flex w-full max-w-4xl h-3/4 bg-pink-50 rounded-2xl shadow-xl border border-pink-100 pane-container">
        {/* Left Messages List */}
      ...
      <div className="absolute bottom-4 w-full text-center text-sm text-pink-400 space-y-1">
        ...
        <div>Version 1.1.0 - <a href="https://github.com/thorintern/thormail" className="hover:text-pink-600 transition-colors" target="_blank">Source code</a></div>
      </div>
    </div>
  );
}
